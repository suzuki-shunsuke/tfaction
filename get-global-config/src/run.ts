...
interface Envs {
    TFACTION_SKIP_ADDING_AQUA_PACKAGES: string
}
...
export const main = (config: lib.Config, input: Input): Result => {
    ...
    const outputs: Outputs = {
        ...
        disable_update_related_pull_requests: !(config?.update_related_pull_requests?.enabled ?? true),
        aqua_update_checksum_enabled: config?.aqua?.update_checksum?.enabled ?? false,
        aqua_update_checksum_prune: config?.aqua?.update_checksum?.prune ?? false,
        aqua_update_checksum_skip_push: input.drift_issue_number ? true : config?.aqua?.update_checksum?.skip_push ?? false,
    ...
    const envs: Envs = {
        TFACTION_SKIP_ADDING_AQUA_PACKAGES: config?.scaffold_working_directory?.skip_adding_aqua_packages ?? "true",
    };
